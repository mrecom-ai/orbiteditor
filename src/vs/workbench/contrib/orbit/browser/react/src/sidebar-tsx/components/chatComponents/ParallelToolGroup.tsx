...
export const ParallelToolGroup = React.memo(({
	...
	const generateSummary = (): string => {
		...
		const toolNameMap: Record<string, (count: number) => string> = {
			...
			'Write': (count) => `Wrote ${count} file${count !== 1 ? 's' : ''}`,
			'edit_file': (count) => `Edited ${count} file${count !== 1 ? 's' : ''}`,
			'rewrite_file': (count) => `Wrote ${count} file${count !== 1 ? 's' : ''}`,
			'create_file_or_folder': (count) => `Wrote ${count} file${count !== 1 ? 's' : ''}`,
			'delete_file_or_folder': (count) => `Ran ${count} command${count !== 1 ? 's' : ''}`,
	...
	return (
		...
		{allToolsCompleted && (
			<div
				className="flex items-center justify-between gap-1.5 text-[13px] font-medium cursor-pointer select-none opacity-80 hover:opacity-100 transition-opacity py-0.5 text-void-fg-4"
