...
class SimpleBrowserManager {
    constructor(extensionUri, automationService) {
        this.extensionUri = extensionUri;
        this.automationService = automationService;
        // Register automation service globally for SimpleBrowserView to access
        if (this.automationService) {
            global.browserAutomationService = this.automationService;
        }
    }
    dispose() {
    ...
    show(inputUri, options) {
        ...
        if (this._activeView) {
            ...
            const view = simpleBrowserView_1.SimpleBrowserView.create(this.extensionUri, url, options, this.automationService);
    ...
    restore(panel, state) {
        const url = state?.url ?? '';
        const view = simpleBrowserView_1.SimpleBrowserView.restore(this.extensionUri, url, panel, this.automationService);
        this.registerWebviewListeners(view);
